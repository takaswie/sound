...
static int ctl_ioctl_elem_read(struct snd_ctl_file *ctl_file, void *buf)
{
...
static int ctl_ioctl_elem_unlock(struct snd_ctl_file *ctl_file, void *buf)
{
	struct snd_ctl_elem_id *id = buf;
	struct snd_kcontrol *kctl;
	struct snd_kcontrol_volatile *vd;
...
static int ctl_ioctl_elem_add(struct snd_ctl_file *file, void *buf)
{
...
static int ctl_ioctl_power_state(struct snd_ctl_file *ctl_file, void *buf)
{
	int *power_state = buf;

#ifdef CONFIG_PM
	*power_state = ctl_file->card->power_state;
#else
